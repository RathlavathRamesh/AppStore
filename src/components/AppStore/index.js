...
class AppStore extends Component {
  ...
  onChangeState = category => {
    this.setState({activTab: category})
  }
  ...
  render() {
    ...
    const displayList = appsList.filter(each => each.category === activTab)
    const finalList = displayList.filter(each =>
      each.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
    return (
      <div className="AppContainer">
        <h1 className="appheading">App Store</h1>
        ...
        <div>
          <ul className="apps">
            {finalList.map(each => (
              <AppItem appinfo={each} key={each.appId} />
            ))}
          </ul>
        </div>
      </div>
